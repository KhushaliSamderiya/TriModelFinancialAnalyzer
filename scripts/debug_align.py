...
print(f"Common companies across all datasets: {len(common_symbols)}")

# --- Limit to 5 Rows per Company ---
...
# --- Align Rows Across Datasets ---
merged_data = fundamental_data.merge(
    sentiment_data, left_on="Symbol", right_on="Ticker", how="inner"
).merge(
    technical_data, on="Ticker", how="inner"
)

print(f"Aligned dataset contains {len(merged_data)} rows.")

# --- Process Aligned Data Points ---
results = []
